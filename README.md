...
4. **Agent 4: Design Overlay**
   ...
   - Uses Affinity Designer MCP (when available)
...
6. **Agent 6: Facebook Integration**
   - Creates draft posts in Facebook Business Manager
   - Uploads final images with captions
   - **Never publishes automatically** (drafts only)
...
- (Optional) Affinity Designer with MCP server
- (Optional) Facebook Business Manager API credentials
...
### Affinity Designer MCP (TODO)

Agent 4 currently uses a placeholder. To enable:

1. Install Affinity Designer MCP server
2. Update `agent_4_design_overlay.py` with MCP calls
3. Configure text styling and layout rules

### Facebook Business Manager (TODO)

Agent 6 currently uses a placeholder. To enable:

1. Create Facebook Business Manager app
2. Get limited-scope API credentials (draft creation only)
3. Store credentials in `.env`
4. Update `agent_6_facebook_integration.py` with API calls
